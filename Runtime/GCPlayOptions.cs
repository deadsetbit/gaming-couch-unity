    ...
    public class GCPlayOptions
    {
        ...
        */
        ...
        public static GCPlayOptions CreateFromJSON(string optionsJson)
        {
            ...
            {
                ValidateProvidedPlayerIndices(transport.players, nameof(optionsJson));
            }
        ...
        private static void ValidatePlayersJsonShape(
            string json,
        ...
        )
        {
            ...
            while (cursor < playersValueEnd)
            {
                ...
                if (elementValueStart < elementEnd && json[elementValueStart] == '{')
                {
                    ...
                    {
                        throw new ArgumentException(LegacyPlayersPayloadErrorMessage, paramName);
                    }

                    if (!ContainsDirectJsonFieldInObject(json, elementValueStart, elementEnd, "playerIndex"))
                    {
                        throw new ArgumentException(MissingPlayerIndexPayloadErrorMessage, paramName);
                    }
                }
        ...
        private static void ValidateProvidedPlayerIndices(GCPlayerOptions[] players, string paramName)
        {
            ...
            for (var index = 0; index < players.Length; index++)
            {
                ...
                {
                    throw new ArgumentException(DensePlayerIndexPayloadErrorMessage, paramName);
                }

                if (seenPlayerIndices[playerIndex])
                {
                    throw new ArgumentException(DuplicatePlayerIndexPayloadErrorMessage, paramName);
                }
