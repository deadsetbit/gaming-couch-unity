...
## Release automation
- CI: `.github/workflows/docfx-unitypackage.yml` builds the DocFX site on a `unity-<version>` tag and deploys the rendered HTML to `deadsetbit/gaming-couch-unity-public` under `latest/`. The build stays here because the site is generated from the package's sources; the deploy goes to the public repo because a private repo on our plan cannot serve Pages. Test classes are staged out of the API reference.
