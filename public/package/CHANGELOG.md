...
- Fixed a seat being one color in DevApp and another in the running game, from seat 4 on. DevApp ordered its seat colors blue, red, green, cyan, yellow, purple, pink, brown, and the package assigned them blue, red, green, yellow, purple, pink, cyan, brown, so a developer testing with four or more players was told one color and shown another. `gc.platform.json` now carries the platform's own order as `properties.colors.seatOrder`, and editor play and the Local Play Settings inspector both read seat colors from it, so neither side keeps a list that has to be held in step by hand. A `gc.platform.json` written before the field existed reports invalid platform data and the run falls back to `GCPlayerColorData.SeatOrder`, which holds the same order, so colors are right either way; pull platform data again in DevApp to clear the warning. `GCPlayerColor`'s declaration order is untouched, because it is not the seat order and reordering it would change every value a game has serialized against it.
- Fixed the package logging seven `CS0414` warnings in every consumer's player build. `GCDevUtils` declared its time scale shortcut settings outside the `#if UNITY_EDITOR` guard that wrapped the only code reading them, so a player build compiled the fields and nothing that used them. The settings now sit behind the same guard. The play mode restart settings escaped the warning only because their reader was unguarded dead code in a player build; both are guarded now, leaving fluctuate FPS as the one part of the class that runs in a build.
- Fixed a build failing when Active Input Handling is set to Input System Package. The editor playtest keyboard and the play-mode restart shortcut read `UnityEngine.Input`, and a runtime check rather than a compile-time one kept them out of a player build, so the legacy calls still had to compile. Both are now compiled out of a player build entirely. A game never reads Unity input for its players anyway. The platform and DevApp deliver it as data.
- The editor playtest keyboard now reads whichever input backend Active Input Handling selects, so editor play works under Old, New, and Both. Under the Input System package it is WASD and the arrow keys, `Space` for primary and `Left Ctrl` for secondary; under the Input Manager it stays on the axis and button names set on the `GamingCouch` component. Set to New with `com.unity.inputsystem` absent, the keyboard is unavailable and the package logs why once per session instead of throwing every frame. `GCDevUtils`' own shortcuts read the IMGUI key event and are unaffected by the setting. That covers time scale, fluctuate FPS and play-mode restart, each of which now acts once per key press rather than repeating while the key is held.
- Fixed the editor playtest keyboard driving a seat the developer did not pick. The number keys and the default selection addressed the game-facing player index, which is a hash of the run seed and each seat's stable key, so index 0 landed on a different seat whenever the seed changed. With DevApp's default random seed that is every run, and it could land on a bot. The keys now pick the seat as DevApp numbers and colors it, so seat 1 is seat 1 in both windows and the arrow keys drive it on every run whatever the seed. The seat number comes from the same rule the runtime uses to build the seat list DevApp reads, so the two cannot drift. A run whose first seat is disabled in DevApp starts on the lowest seat it does have, because a disabled seat keeps its number. Selecting a seat says which seat, color and type it took, and a seat the run does not have says so instead of leaving the keyboard silently where it was.
- Fixed the Start Screen calling a correctly set up scene broken while it plays. Entering Play Mode runs `GamingCouch.Awake`, which marks the object `DontDestroyOnLoad` and moves it out of the active scene. The checklist read only the active scene's root objects, so it reported the GamingCouch missing and dropped the Game script and player prefab rows to blocked behind it, on a scene that was in fact set up correctly. It also offered to create a GamingCouch that already existed, and accepting really did create a second one, which found the original, logged "GamingCouch instance already exists", and destroyed itself in the same frame. The checklist now counts a GamingCouch that Play Mode moved out of the active scene.
- Fixed the Start Screen's scene setup actions running during Play Mode, where a scene change is discarded when Play Mode ends. Create GamingCouch, Create & Wire Game, Wire Player Prefab and "Set up missing pieces" now say to exit Play Mode first, which also stops "Set up missing pieces" rewriting Build Settings and the Game View aspect mid-play. Create New Example Scene and Wire Example Game already refused this way.
- Fixed the Start Screen discarding an action's result unless it was a warning or an error. A setup action that succeeded reported only to the Console, so pressing the button looked like nothing happened. Every result is drawn in the window now.
