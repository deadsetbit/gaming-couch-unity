...
- Fixed the editor playtest keyboard driving a seat the developer did not pick. The number keys and the default selection addressed the game-facing player index, which is a hash of the run seed and each seat's stable key, so index 0 landed on a different seat whenever the seed changed. With DevApp's default random seed that is every run, and it could land on a bot. The keys now pick the seat as DevApp numbers and colours it, so seat 1 is seat 1 in both windows and the arrow keys drive it on every run whatever the seed. The seat number comes from the same rule the runtime uses to build the seat list DevApp reads, so the two cannot drift. Selecting a seat says which seat, colour and type it took, and a seat the run does not have says so instead of leaving the keyboard silently where it was.
