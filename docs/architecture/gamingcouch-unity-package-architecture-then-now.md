...
Scope: `adeaaf5` to `bcca403`

## Purpose

Show how the Unity package architecture moved from broad orchestrators toward smaller modules around the **Local Play Contract**, **Capture**, **Start Screen Readiness**, **Active Scene Setup**, **Example Assets**, DevApp runtime messages, and module-owned tests.
...
flowchart LR
  subgraph Then["Then: broad orchestration at adeaaf5"]
    ...
    Setup["GamingCouchActiveSceneSetup.cs<br/>generated scripts, prefabs,<br/>continuation rules"]
    BroadTests["GamingCouchStartScreenEditorSmokeTests.cs<br/>start screen and active-scene<br/>smoke coverage"]
  end
...
flowchart LR
  ...
  subgraph EditorUi["Editor UI and Start Screen"]
    ...
    SetupEffects["Active Scene Setup, Example Assets,<br/>Build Settings, Game View, WebGL effects"]
...
| Active Scene Setup and Example Assets | `GamingCouchActiveSceneSetup.cs` owned generated scripts, prefabs, scenes, and continuation rules, with broad tests. | `GamingCouchActiveSceneSetup.cs` owns **Example Assets** and current-scene setup side effects, now called through the action runner and tested by asset-focused tests. | Keep active-scene side effects and generated Example Asset rules in one setup module. |
...
| Editor tests | `GamingCouchStartScreenEditorSmokeTests.cs` was a broad fixture covering many modules. | Focused test files mirror contract, session, readiness, messages, setup actions, assets, and smoke behavior; `GamingCouchStartScreenEditorSmokeTests.cs` keeps only cross-module smoke coverage. | Test files follow production module ownership. |
