...
| Task 7 | Runtime message schema and emission; Unity terminal-placement submission behavior; Unity HUD/screen-space runtime output; Unity play payload boot-output options | Adds strict Unity-emitted `runtime_messages` records for `gc.state.snapshot`, player transition messages, diagnostics sequencing continuity, and effectful `gc.game.terminal_placement_submitted` object payloads; coalesces pending state snapshots into late-frame batches; flushes a final snapshot before terminal placement; enforces first-accepted-wins terminal placement in the Unity package before legacy WebGL/DevApp game-over publishing; adds validated `screen_space` envelopes for `playerOverhead` and `playerPosition` anchors while leaving deprecated name-tag points out of the v1 screen-space contract; and adds `runtimeOutput` boot toggles for snapshots and screen-space with transitions, diagnostics, and effectful terminal messages remaining enabled. No DevApp/client/hosted sink UI, Unity log capture, metadata projection, generated example contract, HUD redesign, or `gameProtocolVersion` change. | Yes | Task 8 owns diagnostics sinks and Unity log capture. Task 10 owns HUD rendering redesign on top of the canonical runtime output. Cross-repo adapters still own authoritative receiver-side validation, private player-index/platform-ID mapping, and hosted/client ingestion of these v1 outputs. |
| Task 8 | Unity launch/output configuration; DevApp runtime diagnostics protocol and UI; hosted Unity runtime-message callback validation; captured Unity log diagnostic records | Adds launch-controlled Unity log capture modes (`off`, `error_only`, `warning_and_error`, `full`) that wrap eligible Unity logs as bounded `gc.diagnostic` runtime messages using the active-run clock; adds DevApp `runtime_output_options` control, `runtime_diagnostics` websocket state, active-run diagnostic validation, aggregation, throttled UI updates, filtering/search/clear controls, and a hidden diagnostics rail section; bumps the DevApp controller websocket binary protocol version from 9 to 10 for the new runtime diagnostics message/action codes; and validates hosted `window.gamingCouchRuntimeMessages` diagnostics before forwarding player-index-only records through the client frame event path. No public Unity game API, generated example contract, metadata projection, HUD rendering change, hosted diagnostics UI, or `gameProtocolVersion` change. | Yes | Unity log capture remains development launch policy and defaults off. Hosted public diagnostics exposure is callback/event level only; hosted diagnostics UI remains future work. |
| Task 9 | Public Unity `GCPlayOptions` metadata projection; Unity Editor local-play platform metadata validation; metadata diagnostics runtime behavior | Adds a read-only normalized `GCPlatformRuntimeView` on Unity play options with schema version, validation health, fallback state, source details, game/platform identity, selected entry, sorted entries, and supported player color variants. Missing, invalid, malformed-color, or non-Unity `gc.platform.json` data now reaches local runtime code as the exact `notdefined` fallback and emits `gc.metadata.missing_platform_data` or `gc.metadata.invalid_platform_data` plus `gc.metadata.fallback_active` warning diagnostics through `runtime_messages`; Unity still never writes or repairs `gc.platform.json`. No DevApp/client/SDK protocol, generated example contract, HUD rendering, hosted strict validation, or `gameProtocolVersion` change. | Yes | Hosted production metadata enforcement remains a rollout/upload-validation concern. Task 13 owns documentation updates and Task 14 owns broader cross-repo validation. |
