...
| Task 5 | Replace ambiguous player state APIs with explicit permanent/revokable state. | Not started | Old generic state methods fail at source level, permanent/revokable elimination and finish methods are implemented, new game-facing timestamps use scaled Unity game time, invalid transitions emit diagnostics and no-op, and post-game-over mutations no-op. | Task 0, Task 4, Task 15 | Current elimination is a reversible boolean and finish is a single timestamp. |
...
| Task 7 | Add strict `runtime_messages` and `screen_space` output. | Not started | Unity emits validated `runtime_messages` for state snapshots, transitions, and effectful terminal placement with exact v1 schemas, first-accepted-wins terminal behavior, and validated `screen_space` anchors for overhead and player-position coordinates. | Task 0, Task 1, Task 5, Task 15 | Output uses active-run context, sequence, runtime-relative timing, boot-time optional output toggles, and strict per-type validation. |
...
| Task 15 | Install core runtime diagnostics emitter and ordering infrastructure before API/state/metadata migrations. | Not started | GC diagnostics can be emitted as `gc.diagnostic` runtime messages with package-log-level bypass, warning/error Unity console mirroring, active-run `sequence` and `runtimeTimeMs`, and validated source-area/code payloads before state, metadata, or unsupported API work depends on diagnostics. | Task 0, Task 1 | This task intentionally runs before Tasks 5, 9, and 12 despite the appended stable ID; Task 8 owns DevApp sinks, hosted callback exposure, and optional Unity log capture. |
