...
- Boot-time profiling configuration may disable Unity-originated diagnostics, Unity log capture, `screen_space`, state snapshots, WebGL loader mirroring, and browser console capture. Transition messages remain enabled by default, and effectful messages cannot be disabled because they carry platform-state submissions.
...
| Task 7 | Add strict `runtime_messages` and `screen_space` output. | Not started | Unity emits validated `runtime_messages` for state snapshots, transitions, diagnostics, captured Unity log records when enabled, and effectful terminal placement; Unity emits validated `screen_space` anchors for overhead and player-position coordinates. | Task 0, Task 1, Task 5 | Output uses active-run context, sequence, runtime-relative timing, boot-time optional output toggles, and strict per-type validation; diagnostics and captured Unity logs integrate through this path in Task 8. |
| Task 8 | Add structured GC API diagnostics and development log capture. | Not started | GC API misuse, legacy bridge use, invalid state transitions, metadata fallback, and unsupported API access emit stable current-vocabulary diagnostics through `runtime_messages`; optional Unity log capture is launch-controlled, filtered, rate-limited, timestamped with the active-run clock, can opt into full normal-log capture for development, and stays separate from host-owned WebGL/browser console mirroring. | Task 7 | DevApp is visible in v1; hosted diagnostics UI is future work. |
