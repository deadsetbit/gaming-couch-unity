...
| Task 7 | Add strict `runtime_messages` and `screen_space` output. | Not started | Unity extends the Task 15 runtime-message foundation with validated state snapshots, transition messages, effectful terminal placement with exact v1 schemas, first-accepted-wins terminal behavior, validated `screen_space` anchors for overhead and player-position coordinates, batching rules, and boot-time output toggles. | Task 1, Task 5, Task 15 | Output uses active-run context, sequence, runtime-relative timing, and strict per-type validation. |
