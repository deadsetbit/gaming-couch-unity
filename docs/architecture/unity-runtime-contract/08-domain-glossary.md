...
- **Seat**: One-based local development roster slot used by DevApp and Unity Editor local play for controller assignment, display, and captured local play setup. Seats are not game-facing runtime identity. A seat may map to a different Player Index after deterministic shuffle, and sparse seats become dense players at play capture.
- **DevApp Active Seats**: The local DevApp `activeSeats`/`seatIndex` view used for controller assignment and display before local play capture. Active seats are translated into Player Indices before Unity game-facing runtime behavior.
- **GCSeatIdentity**: Unity package internal local-play provenance stored beside `GCPlayOptions.players`. It preserves source `seatIndex`, stable key, label, player type, and color for DevApp/local routing after sparse seats are shuffled into dense Player Indices. It is not a public play payload field.
- **Player Index**: Zero-based, dense, run-scoped Unity runtime participant identity exposed to game code as `playerIndex` and `GCPlayer.Index`. It is assigned from the captured active roster after deterministic shuffle and is frozen until the run restarts. The same human or bot may receive a different index in a later run.
...
- The Unity package boundary accepts current player / `playerIndex` identity only. Current private boot payloads use `players[]` records that Unity exposes as `GCPlayOptions.players`; legacy hosted roster entries carrying `playerId` or `name` must stay on the client/SDK legacy path or be translated before current Unity package code is invoked.
- The Unity package preserves source-seat mapping to Player Index for local editor play after capturing Local Play Settings through `GCSeatIdentity`. This is current local-play behavior, not hosted Platform Player Id compatibility.
