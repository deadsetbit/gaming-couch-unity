...
- Let Gaming Couch upload and hosted runtime preflight validate Unity builds without launching the Unity loader, wasm, or runtime.
...
- `Runtime/GCRuntimeInfo.cs` hardcodes `PackageName`, `PackageVersion`, `GameProtocolVersion`, and `Platform`.
- Hosted WebGL runtime info is currently emitted from `GamingCouch.Start()` through `GamingCouchRegisterRuntimeInfo`.
- DevApp Editor runtime registration builds `runtime_register` from `GCRuntimeInfo.Create()`.
...
- WebGL builds using the Gaming Couch export template should write `gc.runtime-info.json` at the export root, next to `index.html`.
...
After this Unity package change lands, the Gaming Couch main repo can add upload/client validation that reads `gc.runtime-info.json` before loading Unity. That follow-up should validate sidecar presence, `platform`, SemVer `packageVersion`, minimum supported package version, and supported `gameProtocolVersion`.
