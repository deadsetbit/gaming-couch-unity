...
Next action: Add start-screen WebGL export checklist row.
...
| [x] | DONE | 5. Add menu entry for clean WebGL export setup | Added `GamingCouch/WebGL Build/Setup clean WebGL export`; it runs the setup service, shows dialogs for blockers and warnings, logs actual changes, and stays quiet when setup is already ready with no changes. |
...
- Changed paths: `Editor/Utils/WebBuildOptimizer.cs`, `docs/architecture/gamingcouch-clean-webgl-export-template-prd.md`.
- Added `GamingCouch/WebGL Build/Setup clean WebGL export` to the existing editor-only WebGL build menu helper.
- The menu action calls `GamingCouchWebGLExportSetup.EnsureCleanWebGLExportSetup()`, displays blocker and warning dialogs with details, logs blocker/warning details to the Console, logs successful actual changes, and avoids success dialogs/logs when no changes were needed.
- Verification: inspected menu path placement in `Editor/Utils/WebBuildOptimizer.cs`. `git diff --check` passed.
