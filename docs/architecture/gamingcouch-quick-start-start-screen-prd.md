...
| Task 2 | Validate compatible Game script receivers. | Not started | Assigned listener objects pass readiness when they can receive `GamingCouchSetup(GCSetupOptions)` and `GamingCouchPlay(GCPlayOptions)`, including compatible custom listener objects not named `Game`; broken references fail with manual guidance. | Task 1 | Do not require generated asset names for manually assigned compatible listeners. |
| Task 3 | Implement safe `Create & Wire Game` setup. | Not started | The row action creates or reuses `Assets/Game.cs`, `Assets/Player.cs`, a scene object named `Game`, and the `Game` component through the staged compile flow, then assigns the listener only when the serialized field is empty. | Task 2 | Do not overwrite generated or user-authored assets and do not create or assign the player prefab from this action. |
| Task 4 | Update generated starter Game and Player assets. | Not started | Generated `Game.cs` demonstrates the required GamingCouch setup and play callbacks, `SetupGameVersus`, `SetupDone`, `SetupPlayers<Player>`, randomized final scores, and `GameOver()`; generated `Player.cs` extends `GCPlayer` and supports the visible color placeholder prefab flow. | Task 3 | Existing `GCQuickStartGame` and `GCQuickStartPlayer` assets remain unmigrated unless already assigned and compatible. |
| Task 5 | Add focused validation for Game script readiness behavior. | Not started | Package-only static inspection and consuming-project Unity validation cover receiver compatibility, broken listener guidance, staged compile continuation, `Game.cs` and `Player.cs` conflict handling, `Create & Wire Game` success feedback, and generated quick-start Play Mode behavior. | Task 1, Task 2, Task 3, Task 4 | Keep validation notes aligned with what this package-only checkout can verify. |
...
- Unity edit-mode tests for readiness states, setup actions, no-overwrite behavior, reference preservation, generated asset reuse, `Game.cs` and `Player.cs` conflict handling, compatible custom listener detection, broken listener reference guidance, Build Settings insertion and ordering, Game View 16:9 detection or selection, WebGL readiness summary, and shared inspector readiness summary.
- Manual Unity validation for staged compile continuation, `Create & Wire Game` success feedback, generated quick-start Play Mode loop, valid local play JSON flow, missing or invalid local play JSON messaging, Game View 16:9 behavior, and clean WebGL setup interaction from the Start Screen.
