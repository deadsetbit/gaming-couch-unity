...
- Listener and player prefab assignments are written only when the serialized fields are genuinely empty.
...
- `git diff --check`.
- Static inspection of Start Screen readiness rows, help-button behavior, row message indentation, action separation, and inspector entry ordering.
...
- Unity edit-mode tests for readiness states, setup actions, no-overwrite behavior, reference preservation, generated asset reuse, Build Settings insertion and ordering, Game View 16:9 detection or selection, WebGL readiness summary, and shared inspector readiness summary.
- Manual Unity validation for staged compile continuation, generated quick-start Play Mode loop, valid local play JSON flow, missing or invalid local play JSON messaging, Game View 16:9 behavior, and clean WebGL setup interaction from the Start Screen.
