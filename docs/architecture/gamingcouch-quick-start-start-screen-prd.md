...
- Generated example assets live under `Assets/GamingCouch/GCExample`.
- Existing generated quick-start script, prefab, and scene constants also resolve under `Assets/GamingCouch/GCExample`; the legacy `GCQuickStartGame` and `GCQuickStartPlayer` type and asset names remain unchanged.
- Generated scripts, prefabs, and scenes are editable project content and are never overwritten on rerun.
- Generated active-scene example scripts are named `GCGameExample.cs` and `GCPlayerExample.cs`, with compiled types `GCGameExample` and `GCPlayerExample`, to avoid root-level file and type conflicts.
- Generated active-scene example scripts include a header block explaining that they are GamingCouch example template files that users should move and rename to fit their project, such as `Game.cs` and `Player.cs`.
...
| Task 4 | Update generated starter Game and Player assets. | Completed | Generated `GCGameExample.cs` demonstrates the required GamingCouch setup and play callbacks, `SetupGameVersus`, `SetupDone`, `SetupPlayers<GCPlayerExample>`, randomized final scores, and `GameOver()`; generated `GCPlayerExample.cs` extends `GCPlayer`, supports the visible color placeholder prefab flow, and both scripts include the example-template header; existing generated quick-start script, prefab, and scene constants resolve under `Assets/GamingCouch/GCExample`. | Task 3 | Existing `GCQuickStartGame` and `GCQuickStartPlayer` assets remain unmigrated unless already assigned and compatible. |
| Task 5 | Add focused validation for Game script readiness behavior. | Completed | Package-local editor tests and static inspection cover receiver compatibility, missing and unresolved listener guidance, staged compile classification, `Assets/GamingCouch/GCExample` script conflict handling, `Create & Wire Game` pending result details, generated active-scene `GCGameExample` / `GCPlayerExample` source structure and header text, and legacy quick-start constants under `GCExample`; consuming-project/manual Unity validation remains documented as pending. | Task 1, Task 2, Task 3, Task 4 | Package-local coverage was added where feasible; consuming-project Play Mode validation remains pending/manual. |
...
- Package-local editor-test coverage in `Tests/Editor/GamingCouchQuickStartEditorTests.cs` for compatible custom receivers, inherited receiver methods, incompatible receiver signatures, missing listener guidance and replacement, unresolved listener guidance, `Assets/GamingCouch/GCExample/GCGameExample.cs` and `Assets/GamingCouch/GCExample/GCPlayerExample.cs` path collision blocking, generated active-scene `GCGameExample.cs`/`GCPlayerExample.cs` source structure and header text, legacy quick-start `GCQuickStartGame`/`GCQuickStartPlayer` constants under `Assets/GamingCouch/GCExample`, quick-start player prefab color wiring, `Create & Wire Game` action labels, pending-compilation warning visibility with formatted pending result details, and silent ready success classification.
- Package-local static source inspection confirms generated active-scene example play-loop structure includes `GamingCouchSetup(GCSetupOptions)`, `GamingCouchPlay(GCPlayOptions)`, `SetupGameVersus`, `SetupDone`, `SetupPlayers<GCPlayerExample>`, color application, a coroutine round, randomized final scores, and `GameOver()`, with parallel checks for the legacy `GCQuickStartGame`/`GCQuickStartPlayer` quick-start source structure.
