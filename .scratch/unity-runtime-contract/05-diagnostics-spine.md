...
Define the structured diagnostics path before API migration and state-model work begin. API deprecation warnings, invalid state warnings, mapping validation failures, metadata fallback warnings, unsupported API warnings, and optional Unity warning/error log capture should all use this spine from the start.

## Stable Contract

- Diagnostics are a stable public core contract from day one for `code`, `severity`, `sourceArea`, timestamp, and context shape.
...
- `code` is a lowercase namespaced string such as `gc.api.deprecated_id` or `gc.mapping.invalid_player_index`.
- `severity` is one of `info`, `warning`, or `error`, based on behavior impact:
  - `error`: runtime output was rejected or blocked.
  - `warning`: behavior degraded, no-oped, fell back, or used deprecated/unsupported API.
  - `info`: notable non-problem diagnostic state.
- `sourceArea` is one of `api`, `mapping`, `state`, `events`, `metadata`, or `unity_log`.
- `emittedAt` is UTC epoch milliseconds generated by the emitting runtime or hosted adapter.
- `runId` is optional but top-level when available. In DevApp local play it identifies the active runtime play run and is not stable across restart.
...
- Initial v1 code catalog:
  ...
  - `gc.state.duplicate_out_of_action`
  - `gc.state.duplicate_back_in_action`
  - `gc.state.back_in_action_after_elimination`
  - `gc.events.malformed_event`
  - `gc.events.unknown_event`
  - `gc.events.invalid_game_over_placements`
...
- Unity runtime batches diagnostics into `runtime_diagnostics` messages with a `diagnostics[]` array.
- Runtime batch flushing happens at end of frame when diagnostics exist.
- DevApp validates `runtime_diagnostics` ingress. A malformed diagnostics batch is rejected as a whole and recorded/logged as an ingress validation warning, but it does not close the runtime connection.
...
- Repeat fingerprints are sink-computed from stable fields: `code`, `sourceArea`, `runId`, `playerIndex`, `mapping.mappingId`, and stable reason/path-like `details` fields.
- Fingerprints exclude `message`, `emittedAt`, occurrence counts, changing offending values, and all `debug` content.
...
- The DevApp runtime ingress shape may add `runtime_diagnostics` without old-DevApp compatibility shims because DevApp is not released yet.
...
- Deprecated `Id`, invalid mapping references, invalid state transitions, metadata fallback, unsupported multiplayer APIs, malformed runtime events, invalid game-over placements, and captured Unity warning/error logs emit planned codes.
- DevApp accepts `runtime_diagnostics`, rejects malformed batches without disconnecting the runtime, aggregates by fingerprint, keeps a 200-row active-run ring buffer, and renders the hidden-by-default virtualized console.
...
- Runtime events/results planning owns the full event catalog and strict event payload schemas.
